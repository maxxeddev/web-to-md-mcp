...
class SpaceshipClient {
     ...
     */
    async getDomains(take = 10, skip = 0) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                throw new McpError(ErrorCode.InternalError, `Spaceship API error: ${error.response?.data?.message || error.message}`);
            }
            throw error;
        }
    }
     ...
     * Register a domain
     */
    async registerDomain(domain, contactId) {
        try {
